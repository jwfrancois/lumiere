// Inject test data via the Zustand store API
(function() {
  // Access the store via the module hot reload registry
  // Since we use Zustand vanilla, we can find the store on window if we expose it
  // Otherwise, we can dispatch a custom event that page.tsx listens for.

  // Simpler: just call the global store if it's exposed.
  if (!window.__lumiereStore) {
    // Try to find it by reading the page's React fiber tree
    const root = document.getElementById('__next') || document.querySelector('main')?.parentElement
    if (!root) return 'no root'
    // Walk fiber tree to find the store — too complex. Just dispatch a synthetic event.
  }
  ...
  function add(id, name, path, kind, opts = {}) {
    const file = makeFile(name, new ArrayBuffer(100), kind === 'audio' ? 'audio/mpeg' : 'video/mp4')
    files.push({
      id,
      file,
      name,
      path,
      kind,
      size: kind === 'video' ? 1_000_000_000 : 8_000_000,
      url: URL.createObjectURL(file),
    })
    metadata[id] = { title: name.replace(/\.[^.]+$/, ''), durationSec: kind === 'video' ? 3600 : 240, ...opts }
  }

  // Movies (LOTR trilogy + standalone)
  ...
  window.__testData = { files, metadata }
  return JSON.stringify({ files: files.length, metadata: Object.keys(metadata).length })
})()
